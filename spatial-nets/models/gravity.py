...
def kwargs_from_vec(
    ...
    """Turns vector x into a dict of kwargs."""
    kwargs = dict()
    # kind of like
    # kwargs = {}
    # kwargs = {key, value for key, value in zip(template.keys(), x)}
...
class GravityModel(Model):
        ...
        if method is None:
            ...
            if constraint is None:
                ...
                aux_constraint = DoublyConstrained(
                    maxiters=maxiters, verbose=verbose, symmetric=symmetric)
    ...
    def transform(self, mat=None, coef=None):
        ...
        """
        if coef is None:
            matrix = self._gravity_matrix(**self.coef_)
        else:
            matrix = self._gravity_matrix(**coef)
        return matrix
