...
class UnconstrainedModel(Model):
    """
    Calculate the unconstrained model normalised by flow.
...
class DoublyConstrained(ConstrainedModel):
    """
    Compute the doubly-constrained model.
    ...
    def transform(self, mat: np.ndarray) -> np.ndarray:
        """Calculate the doubly-constrained flow matrix.

        Returns:
        --------
        prediction : np.ndarray
            doubly-constrained flow matrix
        probabilities : attribute, np.ndarray
            row-stochastic flow probabilities matrix (CHECK)
        """
        if self.symmetric:
            prediction, a, b = simple_sipf(
                mat,
                self.target_rows_,
                self.tol,
                self.maxiters,
...
def simple_ipf(
    ...
    while niter < maxiters:
        b = target_cols / (mat.T @ a)  #  each column by a
        a = target_rows / (mat @ b)  # each row by a
        ...
        if bool_row and bool_col:
            break
    if verbose:
        print(f"Nb iters until convergence: {niter} / {maxiters}")
    return out, a, b
    # return out, a, b
...
def simple_sipf(
    ...
    """ Symmetric iterative proportional fitting procedure."""
    if maxiters < 1:
        raise ValueError("invalid number of maximum iterations")
    N, _ = mat.shape
    if target_rows is None:
        target_rows = np.ones(N)
    d = mat.sum(axis=1)  #  row sums
    smat = np.eye(N)
    niter = 0

    while not np.allclose(d, target_rows, atol=tol):
        # while np.linalg.norm(d-target_rows, 2) > tol:
        d = mat.sum(axis=1)  #  row sums
        bmat = np.diag(target_rows ** (1/2))
        dmat = np.diag(d ** (-1/2))
        smat = bmat * dmat @ smat
        out = smat @ mat @ smat
        niter += 1
        if niter > maxiters:
            print(f'failed to converge in {maxiters}')
    s = np.diag(smat)
    return out, s, s
