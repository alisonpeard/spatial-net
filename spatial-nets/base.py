...
class Model(ABC):
    ...
    def fit_transform(
        self, data: LocationsDataClass, mat: Optional[np.ndarray] = None, symmetric=False
    ) -> np.ndarray:
...
class PValues:
    def __init__(
        ...
        if (mask != sparsity(left)).nnz > 0:
            raise ValueError(
                "the sparsity pattern of the matrices does not match")
    ...
    def save(self, filename: Union[str, os.PathLike]):
        if self.model is None:
            ...
            raise DataNotSet(
                "need to specify whether the calculation was approximate")
